...
export default function HeartCollector({
    ...
    const [message, setMessage] = useState("Tap the floating hearts before they disappear 💖");
    ...
    const startGame = () => {
        ...
        setBestCombo(0);
        setIsUnlocked(false);
        setMessage("Tap the floating hearts before they disappear 💖");
    ...
    useEffect(() => {
        ...
        if (score >= targetScore) {
            setMessage("Well done 💖 You reached the heart goal");
        } else if (score >= targetScore * 0.7) {
            setMessage("Almost there 💕 Just a bit more to pass");
        } else {
            setMessage("Great try! Play again to collect more hearts");
        }
    ...
    const handleCollectHeart = (id: number) => {
        ...
        setHearts((prev) =>
            prev.map((heart) => (heart.id === id ? { ...heart, collected: true } : heart))
        );
        ...
        if (nextCombo >= 6) {
            setMessage(`Great! ${target.emoji} COMBO x${nextCombo} +${gained}`);
        } else {
            ...
            <div className="mb-4 flex flex-col gap-4 rounded-[20px] bg-white/85 p-4 shadow-sm sm:mb-5 sm:rounded-[24px] sm:p-5 md:flex-row md:items-center md:justify-between">
                <div>
                    ...
                    <p className="mt-2 text-xs text-slate-600 sm:text-sm">
                        Tap as many floating hearts as you can before time runs out
                    </p>
                ...
                <div className="grid grid-cols-2 gap-3 sm:flex sm:flex-wrap sm:items-center">
                    ...
                    <div className="rounded-2xl bg-violet-100 px-4 py-3 text-center">
                        ...
                        <p className="text-xl font-bold text-violet-600 sm:text-2xl">x{combo}</p>
                    </div>

                    <div className="rounded-2xl bg-sky-100 px-4 py-3 text-center">
                        <p className="text-[11px] text-sky-700 sm:text-xs">Time Left</p>
                        <p className="text-xl font-bold text-sky-600 sm:text-2xl">{timeLeft}s</p>
                    </div>
