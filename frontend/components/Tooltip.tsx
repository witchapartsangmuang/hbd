...
function resolvedSide(trigger: DOMRect, tooltip: DOMRect, preferred: TooltipSide): TooltipSide {
  ...
  const fits = (s: TooltipSide): boolean => {
    switch (s) {
      ...
      case 'top':
        return trigger.top >= th + GAP
          && cx - tw / 2 >= GAP
          && cx + tw / 2 <= W - GAP;
      case 'bottom':
        return trigger.bottom + th + GAP <= H
          && cx - tw / 2 >= GAP
          && cx + tw / 2 <= W - GAP;
      case 'top-start':
        return trigger.top >= th + GAP
          && trigger.left + tw <= W - GAP;
      case 'bottom-start':
        return trigger.bottom + th + GAP <= H
          && trigger.left + tw <= W - GAP;
      case 'top-end':
        return trigger.top >= th + GAP
          && trigger.right - tw >= GAP;
      case 'bottom-end':
        return trigger.bottom + th + GAP <= H
          && trigger.right - tw >= GAP;
      // horizontal — also check vertical centering
      case 'left':
        return trigger.left >= tw + GAP
          && cy - th / 2 >= GAP
          && cy + th / 2 <= H - GAP;
      case 'right':
        return trigger.right + tw + GAP <= W
          && cy - th / 2 >= GAP
          && cy + th / 2 <= H - GAP;
    }
  };
  ...
  return ([preferred, flipped] as TooltipSide[])
    .reduce((best, s) => space[s] >= space[best] ? s : best, preferred);
}
